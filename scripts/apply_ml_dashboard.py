...
SESSION_PANEL_QUERY_OVERRIDES = {
    ...
    "Historian web evidence": 'max(max_over_time({job="ml",source="ml",kind="session_detection_summary",session_id=~"$session_id"} | json | unwrap observed_historian_web_count [$__range])) or vector(0)',
    "SMB access": 'max(max_over_time({job="ml",source="ml",kind="session_detection_summary",session_id=~"$session_id"} | json | unwrap observed_smb_access_count [$__range])) or vector(0)',
    "OPC UA path": 'max(max_over_time({job="ml",source="ml",kind="session_detection_summary",session_id=~"$session_id"} | json | unwrap observed_opcua_path_count [$__range])) or vector(0)',
    "Write-like": 'max(max_over_time({job="ml",source="ml",kind="session_detection_summary",session_id=~"$session_id"} | json | unwrap observed_write_like_count [$__range])) or vector(0)',
    "Critical write": 'max(max_over_time({job="ml",source="ml",kind="session_detection_summary",session_id=~"$session_id"} | json | unwrap observed_critical_write_count [$__range])) or vector(0)',
    "Process anomaly": 'max(max_over_time({job="ml",source="ml",kind="session_detection_summary",session_id=~"$session_id"} | json | unwrap observed_process_anomaly_count [$__range])) or vector(0)',
    "Process anomalies": 'max(max_over_time({job="ml",source="ml",kind="session_detection_summary",session_id=~"$session_id"} | json | unwrap observed_process_anomaly_count [$__range])) or vector(0)',
    "Write-like events": 'max(max_over_time({job="ml",source="ml",kind="session_detection_summary",session_id=~"$session_id"} | json | unwrap observed_write_like_count [$__range])) or vector(0)',
